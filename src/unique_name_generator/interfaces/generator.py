...
class Generator:
    ...
    def ordered_output(self, input_tags: Set[str], constraints: Constraints) \
            -> List[str]:
        try:
            return to_ordered(self.unordered_output(input_tags, constraints))
        except NotImplementedError as ex:
            raise NotImplementedError("ordered_output not supported by " +
                                      f"generator {self.name()}") from ex
        except Exception as ex:
            raise ex

    def unordered_output(self, input_tags: Set[str], constraints: Constraints)\
            -> Counter[str]:
        raise NotImplementedError("unordered_output not supported by " +
